...
theme_furmanu <- function(base_size = 12,
                          base_family = "IBM Plex Sans, Helvetica, Arial, sans-serif") {
  furmanu_font <- base_family
  furmanu_graph_font <- "IBM Plex Sans Condensed, Helvetica, Arial, sans-serif"
  dark_color <- "#201547"
  medium_color <- "#582c83"
  ...
  ggplot2::theme_minimal(base_size = base_size, base_family = base_family) %+replace%
    ggplot2::theme(
      plot.caption = ggplot2::element_text(
        family = furmanu_font, size = ggplot2::rel(0.7), hjust = 1, color = medium_color),
      plot.title = ggplot2::element_text(
        size = ggplot2::rel(1.6), face = "bold", family = furmanu_font, color = dark_color,
        lineheight = ggplot2::rel(1.5), hjust = 0,
        margin = ggplot2::margin(2, 1, 4, 1, "pt")),
      plot.title.position = "panel",
      plot.caption.position = "plot",
      plot.subtitle = ggplot2::element_text(
        size = ggplot2::rel(1.2), family = furmanu_font, color = medium_color,
        lineheight = ggplot2::rel(1.2), hjust = 0,
        margin = ggplot2::margin(1, 1, 10, 1, "pt")),
      axis.title = ggplot2::element_text(
        size = ggplot2::rel(1.1), family = furmanu_graph_font, color = dark_color),
      axis.text = ggplot2::element_text(
        size = ggplot2::rel(0.9), family = furmanu_graph_font, color = medium_color),
      legend.title = ggplot2::element_text(
        size = ggplot2::rel(0.9), family = furmanu_graph_font, face = "bold", color = dark_color),
      legend.text = ggplot2::element_text(
        size = ggplot2::rel(0.8), family = furmanu_graph_font, color = dark_color, lineheight = 0.9),
      strip.text = ggplot2::element_text(
        size = ggplot2::rel(0.9), family = furmanu_font, face = "plain", color = dark_color,
